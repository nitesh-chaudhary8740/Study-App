...
export const loginUser = async (req,res) =>{
   try {
    ...
    const loggedInUser = await User.findById(foundedUser._id).select("-password -refreshToken")
 ...
 .json(new API_Response(200,{user: loggedInUser,accessToken,refreshToken},"logged in successfully"))
   } catch (error) {
...
export const currentUser = async (req,res) => {
  ...
  return res.status(200).json(new API_Response(200,req.user,"user fetched successfully"))
}
